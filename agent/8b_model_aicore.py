...
SYSTEM_PROMPT = """
你是某在线教育平台的高中数学辅导教师，主要帮助学生理解排列组合、概率和
逻辑推理问题。用户消息中会包含教材背景材料和一道待解答的题目。

回答要求：
1. 先说明题型和需要使用的数学原理。
2. 从题目条件出发独立推导，不直接照搬背景材料中的示例结论。
3. 对存在多个限制条件的问题，明确说明各条件是否有交集，避免重复计算。
4. 分步骤展示公式、代入过程和每一步的含义。
5. 单独给出最终答案，并用一种简短方法进行验算。
6. 最后总结本题涉及的知识点和常见易错点。
7. 如果背景材料因长上下文测试而出现重复，只提取有效知识，不重复输出材料。

回答应准确、清晰、亲切，适合高中阶段学生阅读，避免只给结果不讲过程。
""".strip()
USER_PROMPT = (
    "某校数学社团选出甲、乙、丙、丁、戊共5名学生参加成果展示。"
    "现在要求5人排成一列上台，甲不能站在第一位，乙不能站在最后一位。"
    "请问共有多少种不同的排列方式？请写出完整推导过程并进行验算。"
)
PLACEHOLDER_PARAGRAPH = """
排列组合是高中数学计数原理中的重要内容。解决计数问题前，首先要判断完成一件
事情需要分类还是分步。分类加法计数原理适用于若干种互斥方案，完成总数等于各类
方案数之和；分步乘法计数原理适用于必须依次完成的多个步骤，完成总数等于各步骤
方法数之积。

当若干个不同元素按照一定顺序排成一列时，通常使用排列方法。n个不同元素全部
排列共有n!种，其中n!=n×(n-1)×…×2×1。例如4名不同学生排成一列共有4!=24种。
如果只从n个元素中选取m个进行有序排列，则排列数为A(n,m)=n!/(n-m)!。计算时要
特别注意“选择”和“排序”的区别：组合只关心选中了哪些元素，排列还关心先后顺序。

带有限制条件的排列问题常用直接法或间接法。直接法按照限制条件安排特殊元素，
再排列其他元素；间接法先计算没有限制时的所有情况，再减去不符合要求的情况。
当两个不合法条件可能同时发生时，需要使用容斥原理。设全集为U，两个不合法事件
分别为E1和E2，则合法数量为|U|-|E1|-|E2|+|E1∩E2|。最后一项必须加回来，因为
同时满足两个不合法条件的情况在前面被重复减去了两次。

例如，6名志愿者排成一列，如果指定的一名志愿者不能站在队首，可以先计算6!，
再减去该志愿者固定在队首时其余5人的5!种排列。若同时增加另一个人的位置限制，
就要继续判断两个限制条件能否同时发生，并计算交集。也可以固定特殊人物的位置，
通过分步乘法计数原理直接计算，再与间接法结果相互验证。

完成计算后应进行合理性检查：合法排列数必须是不超过总排列数的非负整数；限制
条件增加后，排列数量通常不会增加；如果两个限制条件相互独立或存在交集，不能只
把两个不合法数量简单相减而忽略重复部分。对于人数较少的问题，还可以枚举特殊
人物的位置组合进行验算，从而发现漏算或重复计算。
""".strip()
