...
用法:
  python test_asr/asr_api_stress.py
  python test_asr/asr_api_stress.py --reference-text "参考文本" --output report.json
"""
...
DEFAULT_API_URL = "http://36.111.82.53:10017/v1/audio/trans"
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DEFAULT_AUDIO = os.path.join(PROJECT_ROOT, "test_asr", "asr_test_audio", "1.wav")

SSL_CTX = ssl.create_default_context()
SSL_CTX.check_hostname = False
SSL_CTX.verify_mode = ssl.CERT_NONE

# WAV 头 nframes 可能损坏（如占位 2^30）；超过此值的时长不可信
...
def load_audio_meta(audio_path: str) -> AudioMeta:
    ...
    try:
        ...
        # 非标准 wav 时按 16kHz 单声道 16bit 粗估（仅用于 RTF 参考）
        sample_rate = 16000
        channels = 1
        sample_width = 2
...
async def phase1_baseline(
    ...
    print("\n" + "=" * 88)
    print("阶段1: 单请求基准测试 (各模型)")
    print("=" * 88)
    print(
        f"\n{'模型':<14} {'轮次':<5} {'延迟(s)':<10} {'RTF':<8} {'字/秒':<10} "
        f"{'TTFB(s)':<10} {'字数':<6} {'CER':<8} {'状态'}"
    )
    print("-" * 88)
    ...
    async with aiohttp.ClientSession() as session:
        for model in models:
            ...
            for r in range(rounds):
                payload = build_payload(audio.b64_data, model=model)
            ...
            print(
                f"  {'↳ 平均':<12} {'':5} {agg.avg_latency:<10.3f} {agg.avg_rtf:<8.3f} "
...
async def phase2_concurrency(
    ...
    print("\n" + "=" * 88)
    print(f"阶段2: 递增并发压测 (模型: {model}, 每级持续 {duration_per_level}s, 超时 {timeout_s}s)")
    print("=" * 88)
    print(
    ...
    print(
        ...
        f"{'平均RTF':<9} {'P95总延迟':<9} {'音频吞吐x':<10} {'错误'}"
    )
    print("-" * 100)
    ...
    for concurrency in concurrency_levels:
        ...
        tag = "✓" if stable else "✗"
        print(
            f"{concurrency:<6} {total:<6} {success_count:<6} {success_rate:<7.1f}% "
            f"{success_qps:<9.3f} {attempt_qps:<9.3f} {asr_agg.avg_rtf:<9.3f} "
...
def print_phase3_summary(
    ...
    # ----- ASR 专项 -----
    print("\n【ASR 专项指标 — 基准 (单请求)】")
    print(f"  音频时长: {audio.duration_sec:.2f}s | 采样率: {audio.sample_rate}Hz | 声道: {audio.channels}")
    for model, stats in baseline.items():
        a = stats["asr"]
        cer_s = f"{a['avg_cer']:.2%}" if a.get("avg_cer") is not None else "N/A"
        realtime = "实时" if a["avg_rtf"] < 1 else "非实时"
        print(f"  {model}:")
        print(f"    延迟: 平均 {a['avg_latency']:.3f}s | P50 {a['p50_latency']:.3f}s | P95 {a['p95_latency']:.3f}s")
        print(f"    RTF:  平均 {a['avg_rtf']:.3f}x ({realtime}) | P95 {a['p95_rtf']:.3f}x")
        print(f"    识别: {a['avg_chars_per_sec']:.1f} 字/秒 | 平均 {a['avg_text_chars']:.0f} 字 | CER {cer_s}")
        if stats.get("sample_text"):
            preview = stats["sample_text"][:60] + ("…" if len(stats["sample_text"]) > 60 else "")
            print(f"    样例: {preview}")
    ...
    if stress_levels:
        ...
        for lv in stress_levels:
            a = lv.asr
            print(
                f"  并发 {lv.concurrency:>2}: RTF={a.avg_rtf:.3f}x | 成功P95延迟={a.p95_latency:.3f}s | "
    ...
    print(
        ...
        f"{'P95总延迟':<10} {'P99总延迟':<10} {'尾部(s)':<8} {'错误'}"
    )
    print("  " + "-" * 92)
    for lv in stress_levels:
        err = ", ".join(f"{k}:{v}" for k, v in lv.error_types.items()) or "-"
        print(
            f"  {lv.concurrency:<6} {lv.success_rate:<7.1f}% {lv.success_qps:<10.3f} "
            f"{lv.attempt_qps:<10.3f} {lv.p95_latency:<10.3f} {lv.p99_latency:<10.3f} "
    ...
    # 结合用户实测数据的建议
    max_rtf = max((lv.asr.avg_rtf for lv in stress_levels if lv.success_count), default=0)
    if max_rtf > 1.5:
...
async def run(args: argparse.Namespace) -> None:
    ...
    print(f"  目标:     {args.url}")
    print(f"  音频:     {audio.path}")
    print(f"  时长:     {audio.duration_sec:.2f}s ({audio.file_kb} KB 文件 / {audio.b64_kb} KB base64)")
    ...
    stress_model = args.stress_model or args.models[0]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="FunASR 语音识别服务压力测试 (含 ASR 专项指标)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--url", default=DEFAULT_API_URL, help="ASR 服务地址")
    parser.add_argument("--audio", default=DEFAULT_AUDIO, help="测试 wav 路径")
    parser.add_argument("--models", nargs="+", default=["funasr-iic", "funasr-nano"], help="基准测试模型列表")
    parser.add_argument("--stress-model", default=None, help="并发压测使用的模型，默认取 models[0]")
    ...
    parser.add_argument("--rounds", type=int, default=3, help="基准每模型重复次数")
    parser.add_argument("--timeout", type=float, default=120, help="单次请求超时(秒)；长音频建议≥120")
    parser.add_argument(
        "--stop-success-rate",
        type=float,
        default=90.0,
