...
# 普通 TTS API：直接调用服务，不经过推理网关，也不需要网关签名。
...
def extract_audio_from_json(value: object) -> Optional[bytes]:
    """兼容 JSON 响应中常见的音频字段名，递归查找 base64/WAV 内容。"""
...
class TTSLadderTester:
    ...
    def make_payload(self, text: str) -> dict[str, object]:
        # 普通 /v1/audio/speech 接口只接收 tts_params，不需要网关路由字段。
        tts_params = {
            "text": text,
        ...
        if self.args.function == "instruct2":
            ...
            tts_params["prompt_text"] = self.args.prompt_text
        return {"tts_params": tts_params}
    ...
    def send_request(
        ...
        try:
            ...
            expects_json_audio = "application/json" in content_type
...
def build_final_report(
    ...
    lines.extend(
        [
            ...
            "- “连接就绪及响应头”是 requests.post 返回响应对象的耗时，包含 TCP 建连、服务端排队、处理和响应头等待，requests 无法无侵入地把这些部分完全拆开。",
...
def parse_args() -> argparse.Namespace:
    ...
    # 普通 API 不需要网关鉴权；function 仅用于选择不同 TTS 参数组合，不会发送到服务端。
    parser.add_argument(
        "--function",
        default="zero_shot",
        choices=["zero_shot", "instruct2", "cross_lingual"],
        help="TTS 参数模式；仅影响 tts_params 的组成",
    )

    # TTS 业务参数：直接放入普通接口的 tts_params。
    ...
    parser.add_argument("--prompt-text", default="这是一段参考文本", help="zero_shot 的 prompt_text")
...
def main() -> int:
    ...
    print("\nTTS 阶梯式并发压测")
    print("请求模式: 普通 API 直连")
    print(f"目标 URL: {args.url}")
    print(f"TTS 参数模式: {args.function}")
    print(f"并发级别: {levels}")
    print(
        f"请求计划: "
