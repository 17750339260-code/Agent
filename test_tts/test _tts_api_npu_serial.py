...
class TextGenerator:
    ...
    def __init__(self):
        ...
        self.short_texts = [
            ...
            #
            "随着人工智能技术的快速发展，智能语音合成（TTS）技术已经广泛应用于各个行业。在电力客服领域，TTS可以自动将文字信息转换为自然流畅的语音，用于播报账单信息、欠费提醒、业务通知等场景。高质量的语音合成不仅需要准确的发音，还需要自然的语调、合理的停顿以及情感表达，以提升客户体验。本次测试旨在验证语音合成网关在高并发压力下的性能表现，包括响应成功率、延迟、首字节时间和实时率等关键指标。我们准备了长度约三百字的测试文本，模拟实际业务中可能出现的客服播报内容。通过阶梯式增加并发请求，观察系统在不同负载下的表现，找出容量拐点，为生产环境的资源规划和优化提供数据支撑。此外，测试还关注音频质量和合成稳定性，确保在任何压力下语音清晰可辨并可以听清楚。"
