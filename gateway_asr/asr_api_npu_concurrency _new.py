...
DEFAULT_AUDIO = os.path.join(SCRIPT_DIR, "1.wav")
...
class RequestResult:
    ...
    request_id: int = 0             # 请求自然序号；未分配时为 0
    burst_id: int = 0               # fixed_requests 场景中的同步批次序号
    ...
    response_header_sec: Optional[float] = None  # 请求开始到收到 HTTP 响应头
    ttfb_sec: float = 0.0           # 从请求开始到收到响应体首字节
    ttft_sec: Optional[float] = None  # 流式响应首次出现有效文本增量的耗时
    ...
    response_body_complete: bool = False  # 是否正常读到响应体 EOF；用于区分空响应与中途断流
    ...
    server_processing_sec: Optional[float] = None  # 仅使用服务器响应头明确上报的处理耗时
    server_timing_source: str = ""
...
class AsrAggMetrics:
    ...
    max_latency: float = 0.0        # 成功请求最大延迟
    avg_ttfb: float = 0.0
    p95_ttfb: float = 0.0
    ttfb_samples: int = 0           # 实际收到响应体首字节的成功样本数
    ttfb_coverage: float = 0.0      # TTFB 样本占成功请求比例（%）
    avg_rtf: float = 0.0            # 平均实时因子
    p50_rtf: float = 0.0
    p95_rtf: float = 0.0
    max_rtf: float = 0.0            # 最慢的一条 RTF
    avg_chars_per_sec: float = 0.0  # 请求内文本产出速率：总字数 / 总请求耗时
    text_throughput_cps: float = 0.0  # 系统文本吞吐：总字数 / 指标统计窗口
    ...
    audio_throughput_x: float = 0.0  # 成功处理的「音频秒数」÷ 指标统计窗口
...
class StressAggMetrics:
    ...
    effective_wall_sec: float = 0.0  # 去除人工等待后的请求活跃窗口；吞吐指标统一使用此口径
    first_byte_window_sec: float = 0.0  # burst 最早发起到最晚首字节/响应头的诊断窗口
    planned_requests: int = 0       # 计划发送的请求数
    completed_requests: int = 0     # 已完成并收集到结果的请求数
    total_requests: int = 0         # 发出请求总数
    success_count: int = 0
    failed_count: int = 0
    success_rate: float = 0.0       # 成功率（%）
    attempt_qps: float = 0.0        # 总请求数 / 请求活跃窗口
    success_qps: float = 0.0        # 成功数 / 请求活跃窗口
    wall_attempt_qps: float = 0.0   # 总请求数 / 实际墙钟时间（包含人工间隔和排空）
    wall_success_qps: float = 0.0   # 成功数 / 实际墙钟时间（包含人工间隔和排空）
    ...
    empty_response_count: int = 0   # 已收到 HTTP 响应但响应体为空的样本数
    empty_response_rate: float = 0.0  # 空响应样本占全部请求的比例（%）
    burst_rounds: int = 0           # 同步 burst 批次数
    full_concurrency_bursts: int = 0  # 满并发 burst 批次数
    observed_peak_inflight: int = 0  # 实际观测到的峰值在途请求数
    avg_ttft_sec: Optional[float] = None
    p95_ttft_sec: Optional[float] = None
    ttft_samples: int = 0
    ttft_coverage: float = 0.0
    total_completion_tokens: int = 0
    token_usage_coverage: float = 0.0
    output_token_throughput: Optional[float] = None
    avg_server_processing_sec: Optional[float] = None
    p95_server_processing_sec: Optional[float] = None
    server_processing_samples: int = 0
    server_processing_coverage: float = 0.0
...
def extract_stream_text(payload_text: str) -> str:
    """仅从结构化 JSON 流事件中提取文本，忽略非 SSE 的普通文本行。"""
    ...
    if not payload_text.startswith(("{", "[")):
        return ""
    try:
        event_body = json.loads(payload_text)
    except json.JSONDecodeError:
        return ""
    return extract_text(event_body)


def find_usages(body: Any) -> list[dict[str, Any]]:
    """按响应顺序递归收集 usage；流式响应的最终 usage 通常位于列表末尾。"""
    ...
    if isinstance(body, dict):
        ...
        for value in body.values():
            usages.extend(find_usages(value))
...
def find_usage(body: Any) -> Optional[dict[str, Any]]:
    """返回最后一个 usage，避免误取流式中间事件的不完整累计值。"""
    usages = find_usages(body)
...
def _first_usage_int(usage: dict[str, Any], keys: tuple[str, ...]) -> Optional[int]:
    for key in keys:
        value = _to_optional_int(usage.get(key))
        if value is not None:
            return value
    return None


def extract_usage(body: Any) -> tuple[Optional[int], Optional[int], Optional[int]]:
    """按 235B 并发脚本相同口径提取输入、输出和总 token 数。"""
    selected: Optional[tuple[Optional[int], Optional[int], Optional[int]]] = None
    for usage in find_usages(body):
        ...
        candidate = (prompt_tokens, completion_tokens, total_tokens)
        # 始终保留最后一个含 completion 的完整候选；末尾只有空 usage 时不覆盖它。
        if completion_tokens is not None or selected is None:
            selected = candidate
    return selected or (None, None, None)


def extract_server_processing_time(headers: Any) -> tuple[Optional[float], str]:
    """读取服务器明确上报的处理耗时；不使用客户端耗时反推服务器内部时间。"""
    explicit_ms_headers = (
        "X-Process-Time-Ms",
    ...
    for header_name in explicit_ms_headers:
        raw_value = headers.get(header_name)
        if raw_value in (None, ""):
            continue
        try:
            duration_ms = float(str(raw_value).strip())
        except ValueError:
            continue
        if math.isfinite(duration_ms) and duration_ms >= 0:
            return duration_ms / 1000.0, header_name
    ...
    for entry in server_timing.split(","):
        ...
        for part in parts[1:]:
            if not part.lower().startswith("dur="):
                continue
            try:
                parsed = float(part.split("=", 1)[1].strip().strip('"'))
            except ValueError:
                continue
    ...
    # 多个 Server-Timing 项可能互相包含，不能直接求和；优先采用明确表示总耗时的项。
    priority_names = ("total", "server", "processing", "process", "app", "inference")
    for preferred_name in priority_names:
        for metric_name, duration_ms in reversed(metrics):
            if metric_name == preferred_name:
                return duration_ms / 1000.0, f"Server-Timing:{metric_name}"

    # 非标准命名也保留第一个有效样本，避免多指标响应头全部丢失。
    metric_name, duration_ms = metrics[0]
    return duration_ms / 1000.0, f"Server-Timing:{metric_name}"
...
def aggregate_asr(
    ...
    ttfbs = [r.ttfb_sec for r in successes if r.response_bytes > 0]
    ...
    return AsrAggMetrics(
        ...
        avg_ttfb=statistics.mean(ttfbs) if ttfbs else 0.0,
        p95_ttfb=percentile(ttfbs, 95) if ttfbs else 0.0,
        ttfb_samples=len(ttfbs),
        ttfb_coverage=len(ttfbs) / len(successes) * 100,
...
def build_stress_metrics(
    ...
    effective_wall_sec: Optional[float] = None,
    first_byte_window_sec: float = 0.0,
    burst_rounds: int = 0,
    full_concurrency_bursts: int = 0,
    observed_peak_inflight: int = 0,
) -> StressAggMetrics:
    """根据原始请求列表，统计成功率/QPS，并调用 aggregate_asr 填 asr 字段"""
    successes = [r for r in results if r.success]
    sent_results = [r for r in results if r.start_ts > 0]
    network_total_times = [
    ...
    empty_response_count = sum(
        ...
        and r.response_body_complete
        and r.response_bytes == 0
        and r.response_completion_sec is None
    )
    total = len(results)
    planned = total if planned_requests is None else planned_requests
    qps_window = wall_sec if effective_wall_sec is None else effective_wall_sec
    success_count = len(successes)
    success_rate = (success_count / planned * 100) if planned else 0.0
    ttft_times = [r.ttft_sec for r in successes if r.ttft_sec is not None]
    server_processing_times = [
        r.server_processing_sec
        for r in successes
        if r.server_processing_sec is not None
    ]
    completion_tokens = [
        r.completion_tokens for r in successes if r.completion_tokens is not None
    ]
    total_completion_tokens = sum(completion_tokens)
    token_usage_coverage = (
        len(completion_tokens) / success_count * 100 if success_count else 0.0
    )
    usage_complete = bool(successes) and len(completion_tokens) == success_count
    ...
    return StressAggMetrics(
        ...
        wall_sec=wall_sec,
        effective_wall_sec=qps_window,
        first_byte_window_sec=first_byte_window_sec,
        ...
        attempt_qps=len(sent_results) / qps_window if qps_window > 0 else 0.0,
        success_qps=success_count / qps_window if qps_window > 0 else 0.0,
        wall_attempt_qps=len(sent_results) / wall_sec if wall_sec > 0 else 0.0,
        wall_success_qps=success_count / wall_sec if wall_sec > 0 else 0.0,
        ...
        empty_response_rate=(empty_response_count / total * 100 if total else 0.0),
        burst_rounds=burst_rounds,
        full_concurrency_bursts=full_concurrency_bursts,
        observed_peak_inflight=observed_peak_inflight,
        avg_ttft_sec=statistics.mean(ttft_times) if ttft_times else None,
        p95_ttft_sec=percentile(ttft_times, 95) if ttft_times else None,
        ttft_samples=len(ttft_times),
        ttft_coverage=(len(ttft_times) / success_count * 100 if success_count else 0.0),
        total_completion_tokens=total_completion_tokens,
        token_usage_coverage=token_usage_coverage,
        output_token_throughput=(
            total_completion_tokens / qps_window
            if qps_window > 0 and usage_complete
            else None
        ),
        avg_server_processing_sec=(
            statistics.mean(server_processing_times) if server_processing_times else None
        ),
        p95_server_processing_sec=(
            percentile(server_processing_times, 95) if server_processing_times else None
        ),
        server_processing_samples=len(server_processing_times),
        server_processing_coverage=(
            len(server_processing_times) / success_count * 100 if success_count else 0.0
        ),
        error_types=error_types,
        extra=extra_metrics,
        asr=aggregate_asr(successes, audio_duration, qps_window),
    )
...
def print_stress_metrics(m: StressAggMetrics, *, focus: str = "") -> None:
    ...
    if not math.isclose(m.effective_wall_sec, m.wall_sec, rel_tol=1e-9, abs_tol=1e-9):
        print(f"  │ 请求活跃窗口: {m.effective_wall_sec:.2f}s（吞吐计算口径）")
    if m.first_byte_window_sec > 0:
        print(f"  │ 首字节窗口:   {m.first_byte_window_sec:.2f}s（仅诊断，不计算吞吐）")
    print(f"  │ 总请求:       {m.total_requests} | 成功: {m.success_count}")
    print(f"  │ 成功率:       {m.success_rate:.2f}%")
    print(f"  │ 活跃窗口吞吐: 尝试 {m.attempt_qps:.3f} QPS | 成功 {m.success_qps:.3f} QPS")
    if not math.isclose(m.effective_wall_sec, m.wall_sec, rel_tol=1e-9, abs_tol=1e-9):
        print(
            f"  │ 墙钟吞吐:     尝试 {m.wall_attempt_qps:.3f} QPS | "
            f"成功 {m.wall_success_qps:.3f} QPS"
        )
    ...
    print(
        f"  │ 空响应样本占比: {m.empty_response_count}/{m.total_requests} "
        f"({m.empty_response_rate:.1f}%)"
    )
    print(
        "  │ 服务器处理耗时(响应头上报): "
        f"平均 {_format_optional_ms(m.avg_server_processing_sec)} | "
        f"P95 {_format_optional_ms(m.p95_server_processing_sec)} | "
        f"样本 {m.server_processing_samples}/{m.success_count} "
        f"({m.server_processing_coverage:.1f}%)"
    )
    if m.error_types:
...
def print_asr_metrics(a: AsrAggMetrics, audio: AudioMeta, *, sample_text: str = "") -> None:
    ...
    print(
        f"  │ 首字节响应时间(TTFB): 平均 {a.avg_ttfb:.3f}s | P95 {a.p95_ttfb:.3f}s | "
        f"样本 {a.ttfb_samples} ({a.ttfb_coverage:.1f}%)"
    )
    ...
    print(f"  │ 文本吞吐:     {a.text_throughput_cps:.1f} 字/统计秒 | 请求内 {a.avg_chars_per_sec:.1f} 字/请求秒")
    print(f"  │ 音频语速:     {a.audio_chars_per_sec:.1f} 字/音频秒 | 平均 {a.avg_text_chars:.0f} 字/条")
    print(f"  │ 空结果率:     {a.empty_text_rate:.2f}%")
    print(f"  │ 音频吞吐:     {a.audio_throughput_x:.3f}x (成功音频秒/统计秒)")
    ...
    *,
    ...
    try:
        async with session.post(
            ...
            result.response_header_sec = max(time.perf_counter() - start, 0.0)
            (
                result.server_processing_sec,
                result.server_timing_source,
            ) = extract_server_processing_time(resp.headers)
            first_body_byte_at: Optional[float] = None
            last_body_byte_at: Optional[float] = None
            chunks: list[bytes] = []
            stream_line_buffer = b""
            stream_event_data_lines: list[str] = []

            def observe_stream_payload(payload_text: str, observed_at: float) -> None:
                """仅用于记录流式有效文本首包时间，不改变最终响应解析结果。"""
                if result.ttft_sec is not None:
                    return
                if extract_stream_text(payload_text):
                    result.ttft_sec = max(observed_at - start, 0.0)

            def observe_stream_line(raw_line: bytes, observed_at: float) -> None:
                line = raw_line.decode("utf-8", errors="replace").rstrip("\r")
                if not line:
                    if stream_event_data_lines:
                        observe_stream_payload("\n".join(stream_event_data_lines), observed_at)
                        stream_event_data_lines.clear()
                    return
                if line.startswith("data:"):
                    data_line = line[5:].lstrip()
                    stream_event_data_lines.append(data_line)
                    # 常见 SSE 事件只有一行 data；在该行到达时立即记录 TTFT。
                    observe_stream_payload(data_line, observed_at)
                elif not line.startswith(":"):
                    # 兼容 newline-delimited JSON；普通文本行不作为 TTFT 样本。
                    observe_stream_payload(line, observed_at)

            # 与参考脚本相同，先单独读取 1 字节，避免把整块到达时间误当作首字节时间。
            first_byte = await resp.content.read(1)
            if first_byte:
                first_body_byte_at = time.perf_counter()
                last_body_byte_at = first_body_byte_at
                result.ttfb_sec = max(first_body_byte_at - start, 0.0)
                chunks.append(first_byte)
                result.response_bytes += len(first_byte)
                if response_stream is True:
                    stream_line_buffer += first_byte
            ...
            async for chunk in resp.content.iter_chunked(8192):
                if chunk:
                    ...
                    if response_stream is True and result.ttft_sec is None:
                        stream_line_buffer += chunk
                        while b"\n" in stream_line_buffer:
                            raw_line, stream_line_buffer = stream_line_buffer.split(b"\n", 1)
                            observe_stream_line(raw_line, chunk_received_at)

            result.response_body_complete = True

            if response_stream is True and result.ttft_sec is None:
                observed_at = last_body_byte_at or time.perf_counter()
                if stream_line_buffer:
                    observe_stream_line(stream_line_buffer, observed_at)
                if stream_event_data_lines:
                    observe_stream_payload("\n".join(stream_event_data_lines), observed_at)
...
def assign_request_ids_in_start_order(results: list[RequestResult]) -> None:
    """按实际发起时间补充分场景请求序号；无 burst 的场景保留 burst_id=0。"""
    for request_id, result in enumerate(
        sorted(results, key=lambda item: item.start_ts),
        start=1,
    ):
...
async def run_baseline(
    ...
    async with aiohttp.ClientSession() as session:
        for model in models:
            ...
            request_active_sec = sum(res.latency_sec for res in results)
            m = build_stress_metrics(
                TestKind.BASELINE.value,
                results,
                wall,
                ctx.audio.duration_sec,
                concurrency=1,
                effective_wall_sec=request_active_sec,
            )
            m.extra["model"] = model
            m.extra["rounds"] = rounds
            successful_active_sec = sum(res.latency_sec for res in results if res.success)
            m.extra["请求活跃耗时"] = f"{request_active_sec:.3f}s"
            m.extra["成功请求活跃耗时"] = f"{successful_active_sec:.3f}s"
            out[model] = m
...
async def run_spike(
    ...
    m.extra["尖峰全部请求P95延迟"] = f"{spike_m.p95_network_total_sec:.3f}s"
...
async def run_soak(
    ...
    async def worker(session: aiohttp.ClientSession) -> None:
        while time.perf_counter() < stop_at:
            ...
            async with lock:
                results.append(res)
                # 按请求发起时刻分桶，避免慢请求因自身延迟被系统性推入后续桶。
                elapsed = max(res.start_ts - start, 0.0)
...
async def run_accuracy_load(
    ...
    if successes:
        ...
        # allow_empty_text=True 时也保留空文本，避免一致性分母只统计非空成功样本。
        texts = [r.text for r in successes]
        if texts:
...
class AsyncStartGate:
    ...
    async def ready_and_wait(self) -> None:
        ...
        try:
            async with self.condition:
                await asyncio.wait_for(
                    self.condition.wait_for(lambda: self.ready >= self.target_ready),
                    timeout=timeout,
                )
            return True
        except asyncio.TimeoutError:
...
def calculate_effective_burst_duration(results: list[RequestResult]) -> float:
    """累加每个 burst 从最早发起到最后完成的请求活跃窗口，排除批次间人工空闲。"""
    sent_results = [r for r in results if r.start_ts > 0 and r.end_ts >= r.start_ts]
    bursts: dict[int, list[RequestResult]] = {}
    for result in sent_results:
        bursts.setdefault(result.burst_id, []).append(result)

    effective_duration = 0.0
    for burst_results in bursts.values():
        burst_start = min(r.start_ts for r in burst_results)
        burst_end = max(r.end_ts for r in burst_results)
        effective_duration += max(burst_end - burst_start, 0.0)
    return effective_duration


def calculate_first_byte_burst_duration(results: list[RequestResult]) -> float:
    """累加 burst 的首响应窗口；仅用于诊断并发响应启动速度，不作为吞吐分母。"""
    sent_results = [r for r in results if r.start_ts > 0 and r.end_ts >= r.start_ts]
    bursts: dict[int, list[RequestResult]] = {}
    for result in sent_results:
        bursts.setdefault(result.burst_id, []).append(result)

    first_byte_duration = 0.0
    for burst_results in bursts.values():
        burst_start = min(r.start_ts for r in burst_results)
        response_start_times = [
            r.start_ts + r.ttfb_sec
            for r in burst_results
            if r.response_bytes > 0 and r.ttfb_sec >= 0
        ]
        # 空响应没有响应体首字节，使用已观测到的响应头到达时间作为保守回退。
        response_start_times.extend(
            r.start_ts + r.response_header_sec
            for r in burst_results
            if r.response_bytes == 0
            and r.response_header_sec is not None
            and r.response_header_sec >= 0
        )
        # 未收到任何响应时只保留发起抖动；该值只用于诊断，不参与吞吐计算。
        burst_end = max(response_start_times, default=max(r.start_ts for r in burst_results))
        first_byte_duration += max(burst_end - burst_start, 0.0)
    return first_byte_duration


def _format_optional_ms(value_sec: Optional[float]) -> str:
    return "N/A" if value_sec is None else f"{value_sec * 1000:.2f}ms"
...
def print_fixed_requests_report(m: StressAggMetrics) -> None:
    """使用 235B 并发脚本相同的指标名称、计算口径和展示顺序。"""
    success_latency_available = m.success_count > 0
    success_avg = m.asr.avg_latency if success_latency_available else None
    success_p50 = m.asr.p50_latency if success_latency_available else None
    success_p95 = m.asr.p95_latency if success_latency_available else None
    success_p99 = m.asr.p99_latency if success_latency_available else None
    success_max = m.asr.max_latency if success_latency_available else None
    all_request_available = m.completed_requests > 0
    all_avg = m.avg_network_total_sec if all_request_available else None
    all_p95 = m.p95_network_total_sec if all_request_available else None
    all_p99 = m.p99_network_total_sec if all_request_available else None
    ttfb_avg = m.asr.avg_ttfb if m.asr.ttfb_samples > 0 else None
    ttfb_p95 = m.asr.p95_ttfb if m.asr.ttfb_samples > 0 else None

    print(f"\n并发 {m.concurrency} 测试结果")
    print(f"计划请求数: {m.planned_requests}")
    print(f"完成请求数: {m.completed_requests}")
    print(f"成功/失败: {m.success_count}/{m.failed_count} ({m.success_rate:.2f}%)")
    print(
        f"同步批次: {m.burst_rounds}, "
        f"满并发批次: {m.full_concurrency_bursts}/{m.burst_rounds}"
    )
    print(f"目标并发/客户端峰值在途请求: {m.concurrency}/{m.observed_peak_inflight}")
    print(f"总耗时: {m.wall_sec:.2f}s")
    print(f"请求活跃窗口: {m.effective_wall_sec:.2f}s (排除批次间人工等待)")
    print(f"首字节窗口: {m.first_byte_window_sec:.2f}s (仅诊断，不用于吞吐)")
    print(f"活跃窗口总QPS/成功QPS: {m.attempt_qps:.2f}/{m.success_qps:.2f}")
    print(f"墙钟总QPS/成功QPS: {m.wall_attempt_qps:.2f}/{m.wall_success_qps:.2f}")
    print(
        "成功请求响应耗时: "
        f"avg={_format_optional_ms(success_avg)}, "
        f"p50={_format_optional_ms(success_p50)}, "
        f"p95={_format_optional_ms(success_p95)}, "
        f"p99={_format_optional_ms(success_p99)}, "
        f"max={_format_optional_ms(success_max)}"
    )
    print(
        "全部已发送请求端到端耗时: "
        f"avg={_format_optional_ms(all_avg)}, "
        f"p95={_format_optional_ms(all_p95)}, "
        f"p99={_format_optional_ms(all_p99)}"
    )
    print(
        f"空响应样本占比: {m.empty_response_count}/{m.total_requests} "
        f"({m.empty_response_rate:.2f}%)"
    )
    print(
        f"TTFB: avg={_format_optional_ms(ttfb_avg)}, p95={_format_optional_ms(ttfb_p95)} "
        f"(样本={m.asr.ttfb_samples}/{m.success_count}, 覆盖率={m.asr.ttfb_coverage:.2f}%)"
    )
    print(
        "TTFT(stream文本首包): "
        f"avg={_format_optional_ms(m.avg_ttft_sec)}, "
        f"p95={_format_optional_ms(m.p95_ttft_sec)} "
        f"(样本={m.ttft_samples}/{m.success_count}, 覆盖率={m.ttft_coverage:.2f}%)"
    )
    print(
        "输出 token 吞吐: "
        f"{_format_optional_number(m.output_token_throughput, ' tok/s')} "
        f"(仅 usage 完整覆盖时计算，usage覆盖率={m.token_usage_coverage:.2f}%)"
    )
    print(
        "服务器处理耗时(响应头上报): "
        f"avg={_format_optional_ms(m.avg_server_processing_sec)}, "
        f"p95={_format_optional_ms(m.p95_server_processing_sec)} "
        f"(样本={m.server_processing_samples}/{m.success_count}, "
        f"覆盖率={m.server_processing_coverage:.2f}%)"
    )
    if m.error_types:
        print("失败原因统计:")
        for error, count in sorted(m.error_types.items(), key=lambda item: item[1], reverse=True):
            print(f"  {error}: {count}")
...
async def run_fixed_requests(
    ...
    *,
    ...
    """固定请求数同步 burst 压测，计算口径与 235B 并发脚本保持一致。"""
    print_test_banner(
    ...
    burst_rounds = math.ceil(total_requests / concurrency)
    inflight = AsyncInflightCounter()
    start = time.perf_counter()
    ...
    async def worker(
        ...
        request_id: int,
        burst_id: int,
        start_gate: AsyncStartGate,
    ) -> RequestResult:
        await start_gate.ready_and_wait()
        inflight.enter()
        try:
            payload = build_request_payload(ctx, response_stream=response_stream)
            res = await send_one_request(session, ctx, payload, response_stream=response_stream)
            res.request_id = request_id
            res.burst_id = burst_id
            return res
        finally:
            inflight.leave()
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        next_request_id = 1
        for burst_id in range(1, burst_rounds + 1):
            burst_size = min(concurrency, total_requests - completed)
            start_gate = AsyncStartGate(burst_size)
            tasks = [
                asyncio.create_task(worker(session, request_id, burst_id, start_gate))
                for request_id in range(next_request_id, next_request_id + burst_size)
            ]
            next_request_id += burst_size

            if not await start_gate.wait_until_ready(start_timeout):
                start_gate.release()
            ...
            print(f"第 {burst_id}/{burst_rounds} 批释放: {burst_size} 个请求同时发起")
            start_gate.release()
            ...
            for completed_task in asyncio.as_completed(tasks):
                ...
                if completed % progress_every == 0 or completed == total_requests:
                    ...
                    print(
                        ...
                        f"当前成功率={ok_count / completed * 100:.2f}%"
                    )

            if burst_id != burst_rounds and burst_interval > 0:
                elapsed = time.perf_counter() - burst_start
                sleep_seconds = max(burst_interval - elapsed, 0.0)
                if sleep_seconds > 0:
                    await asyncio.sleep(sleep_seconds)

    wall = time.perf_counter() - start
    effective_wall = calculate_effective_burst_duration(results)
    first_byte_window = calculate_first_byte_burst_duration(results)
    m = build_stress_metrics(
        ...
        ctx.audio.duration_sec,
        concurrency=concurrency,
        extra={"响应模式": "流式" if response_stream else "非流式"},
        planned_requests=total_requests,
        effective_wall_sec=effective_wall,
        first_byte_window_sec=first_byte_window,
        burst_rounds=burst_rounds,
        full_concurrency_bursts=total_requests // concurrency,
        observed_peak_inflight=inflight.peak,
    )
    print_fixed_requests_report(m)
    return m
...
def print_acceptance_summary(
    *,
    ...
    """打印并返回本次音频验收指标判定。"""
    stream_ttfb = (
        stream_probe.asr.avg_ttfb
        if stream_probe and stream_probe.asr.ttfb_samples > 0
        else None
    )
    non_stream_latency = (
    ...
    all_p95_latency = fixed_8x10.p95_network_total_sec if fixed_8x10.total_requests else None
    stable_ok = (
...
async def run(args: argparse.Namespace) -> None:
    ...
    if TestKind.FIXED_REQUESTS.value in tests:
        print(
            "  fixed_requests响应模式: "
            + ("流式" if args.fixed_requests_stream else "非流式")
        )
    if ctx.gateway:
    ...
    report: dict[str, Any] = {
        ...
        "config": {
            ...
            "fixed_requests_stream": args.fixed_requests_stream,
            "burst_interval": args.burst_interval,
            "start_timeout": args.start_timeout,
            "allow_empty_text": ctx.allow_empty_text,
        ...
        "metric_definitions": {
            "wall_sec": "测试阶段实际墙钟时间，包含配置的批次间隔、冷却等待和请求排空",
            "effective_wall_sec": "请求实际活跃到完成的统计窗口；排除人工等待，用于吞吐计算",
            "first_byte_window_sec": "burst 最早发起到最晚首字节或响应头的诊断窗口，不参与吞吐计算",
            "attempt_qps": "已发请求数 / effective_wall_sec",
            "success_qps": "成功请求数 / effective_wall_sec",
            "wall_attempt_qps": "已发请求数 / wall_sec",
            "wall_success_qps": "成功请求数 / wall_sec",
        },
    }
...
def validate_args(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    ...
    if args.start_timeout <= 0:
        parser.error("--start-timeout 必须大于 0")
    if args.burst_interval < 0:
        parser.error("--burst-interval 不能小于 0")
...
def main() -> None:
    ...
    # ---------- ramp 递增并发相关参数 ----------
    parser.add_argument("--concurrency", nargs="+", type=int, default=[1,4,8,12,20,30,40])
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="fixed_requests 同步 burst 之间的最小间隔秒数",
    )
    parser.add_argument(
        "--start-timeout",
        type=float,
        default=180.0,
        help="等待同一 burst 全部 worker 就绪的超时秒数",
    )
