...
# TTS 网关地址及认证信息：与 tts_step_concurrent_new.py 的测试网关保持一致。
...
# HTTP 连接超时 / 读超时（秒）：长文本合成可能需要较长时间，统一设置为 360 秒。
CONNECT_TIMEOUT = 360
READ_TIMEOUT = 360
...
# 流式 chunk 间隔超时：超过 360 秒未收到新数据才判定连接卡死。
CHUNK_TIMEOUT = 360
...
def build_gateway_auth_headers():
    """按网关要求生成当前请求使用的 HMAC-SHA256 签名请求头。"""
    x_date = datetime.now(timezone.utc).strftime("%a, %d %b %Y %T GMT")
    sign_text = f"x-date: {x_date}"
    signature = base64.b64encode(
        hmac.new(
            DEFAULT_GATEWAY_SECRET_KEY.encode("utf-8"),
            sign_text.encode("utf-8"),
            sha256,
        ).digest()
    ).decode("utf-8")
    authorization = (
        f'hmac username="{DEFAULT_GATEWAY_APP_KEY}", algorithm="hmac-sha256", '
...
def build_gateway_payload(text, mode):
    """把串行脚本的模式配置转换成网关 /predict 接口要求的请求体。"""
    mode_params = mode["tts_params"]
    function = mode["model"]
    return {
        "componentCode": DEFAULT_GATEWAY_COMPONENT_CODE,
        "model": DEFAULT_GATEWAY_MODEL,
        "function": function,
        "tts_params": {
            "input_text": text,
            "speaker_id": mode_params.get("zero_shot_spk_id", ""),
            "prompt_audio": mode_params.get("prompt_audio", ""),
            "prompt_text": mode_params.get("prompt_text", ""),
            "instruct_text": (
                mode_params.get("instruct_text", "")
                if function == "instruct2"
                else ""
            ),
            "stream": mode_params.get("stream", True),
            "speed": mode_params.get("speed", 1.0),
            "background_audio": mode_params.get("background_audio", ""),
            "background_volume": mode_params.get("background_volume", 0.0),
            "background_loop": mode_params.get("background_loop", True),
            "text_frontend": mode_params.get("text_frontend", True),
            "seed": mode_params.get("seed", 0),
            "split": mode_params.get("split", True),
            "res_content": True,
            "response_format": "wav",
        },
    }
...
class TextGenerator:
    ...
    def __init__(self):
        ...
        self.short_texts = [
            "认知的套利者，在这个世界大有搞头的逻辑里，最高级的财富，是你的选择权，2011年2月1日，它是智力资本在时间复利中的悄然绽放，它是认知高地对低洼地带的温柔俯瞰。当别人在存量博弈里拼刺刀，你已在 this is the begin 正确非共识 this is the end 的无人区，种下了属于未来的森林。自由的代价，从来不是不被强迫，而是你看得见，万千条通往星辰的隐秘路径。"
        ]
...
def execute_request(result_queue, session, payload, iteration):
    ...
    try:
        # 网关签名中包含当前 GMT 时间，因此每次请求都重新生成请求头。
        headers = build_gateway_auth_headers()
...
def main():
    ...
    while RUNNING:
        ...
        try:
            ...
            payload = build_gateway_payload(text, mode)
